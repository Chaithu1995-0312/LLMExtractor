...
UI_CHAT_MAPPING_PATH = os.path.join("ui", "jarvis", "public", "chat_mapping.json")
...
def run_sync(input_json: str, output_dir: str, rebuild_index: bool = False):
    ...
    try:
        ...
        if processed_count == 0 and not rebuild_index:
            print(f"[{datetime.now(timezone.utc).isoformat()}] No new data processing required.")
            # Still export mapping to ensure consistency
            print(f"[{datetime.now(timezone.utc).isoformat()}] [UI] Exporting chat mapping for UI...")
            conv_index.export_chat_mapping(UI_CHAT_MAPPING_PATH)
            return
